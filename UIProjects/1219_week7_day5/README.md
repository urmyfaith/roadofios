...
| 非专业版 | 专业 |
| ------------- | ------------ |
| 直接买碗吃饭  | 1.要使用碗,先到后厨取碗,有,拿来使用,没有,去买碗|
||内存,内存里如果有数据,未过期,取出数据使用,过期,请求|
...
通过复用机制,可以在只创建几个cell的情况下,将表中所有的数据展示出来,大大的节省了内存

标志符不一样的时候,"缓存去区"里就会出现不同的类型的cell

如果只有一种标志类型的cell===>只存在cell

如果有标志类型的cell==>存在多种cell
