...
//  [self.view addSubview:editButton];
    ...
    self.bar.placeholder = @"search";//提示信息
    self.tableView.tableHeaderView = self.bar;
    
    //9.2 创建搜索类
    /**
     initWithSearchBar: 搜索类与当前类共用一个搜索栏
     contentsController:搜索类需要搜索的内容所在的视图控制器.
     */
    //注意:一定要写全局的搜索类.因为要保证搜索类整个程序都可以使用:可见.
    self.display = [[UISearchDisplayController alloc] initWithSearchBar:self.bar contentsController:self];
    self.display.searchResultsDataSource = self;//搜索结果--->数据源
    self.display.searchResultsDelegate = self;//搜索结果---->显示
    self.searchCells = [NSMutableArray array];
    
}

# pragma mark  editButtonClicked
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if ([tableView isEqual:self.tableView] == NO) {
        ...
        //遍历表的所有内容
        for (NSArray *eachArray in self.datas) {
            for (NSString *eachString in eachArray) {
                if ([eachString rangeOfString:self.bar.text].location != NSNotFound) {
                    [self.searchCells addObject:eachString];
                }
            }
        }
        return  self.searchCells.count;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tableView isEqual:self.tableView] == NO) {
        //搜索栏的输入数据显示
        cell.textLabel.text  = self.searchCells[indexPath.row];
    }
    else{
